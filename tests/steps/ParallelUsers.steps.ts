...
When('user 1 fills and submits the automation practice form with valid data', async function () {
  const context = await this.browser.newContext();
  const page = await context.newPage();
  // Hide ad iframe/banner if present to prevent pointer event interception
  await page.evaluate(() => {
  ...
  if (!foundConfirmation) {
    ...
    const modalHtml = await form.page.locator('.modal-content').innerHTML().catch(() => '');
    // eslint-disable-next-line no-console
    console.log('DEBUG: No confirmation modal. Visible text:', errorText);
    // eslint-disable-next-line no-console
    console.log('DEBUG: Modal HTML:', modalHtml);
    await form.page.screenshot({ path: 'debug-practice-form.png', fullPage: true }).catch(() => {});
  }
  this.formConfirmation = foundConfirmation;
  await context.close();
});

When('user 2 shuffles the sortable grid items', async function () {
  const context = await this.browser.newContext();
  const page = await context.newPage();
...
When('user 1 fills the automation practice form with an invalid email', async function () {
  const context = await this.browser.newContext();
  const page = await context.newPage();
  const form = new PracticeFormPage(page);
  await form.goto();
  ...
  if (!isValid) {
    ...
    try {
      ...
      this.emailError = response.status() === 400 || response.status() === 406;
    }
    await context.close();
    return;
  }
...
Then('user 1 should see the form submission confirmation', async function () {
  expect(this.formConfirmation).toBe(true);
});

Then('user 2 should see the grid items reordered', async function () {
  expect(this.gridOrderChanged).toBe(true);
});

Then('user 1 should see an email validation error', async function () {
